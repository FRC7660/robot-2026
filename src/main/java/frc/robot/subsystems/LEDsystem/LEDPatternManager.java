...
public class LEDPatternManager extends SubsystemBase {
    ...
    public static class LightRoutine {
        ...
        /** Initialization */
        public LightRoutine(Launch launchSystem, Intake intakeSystem, SwerveSubsystem swerveSystem, Turret turretSystem) {
                ...
                case SHOOTER:
                    returnPattern = () -> 
                    {
                        if (routineName == "Test") {
                            ...
                            return pBank.staggerWhite.scrollAtAbsoluteSpeed(MetersPerSecond.of(launch.getVelocity().in(RadiansPerSecond) / 300), kLedSpacing)
                            .overlayOn(
                                pBank.orange.synchronizedBlink(
                                () -> {
                                    // Compares the live velocity to the optimal and flashes orange if they are not close.
                                    return !launch.getVelocity().isNear(launch.getOptimalVelocity(),0.02);
                                }));
                        };
                        return pBank.yellow;
                    };
                    ...
                    returnPattern = () -> 
                    {
                        if (routineName == "Test") {
                            ...
                            if (drivebase.getSwerveDrive().getGyro().getAccel().isEmpty()) {
                                // If the gyro is not providing acceleration data, it is likely disconnected or malfunctioning, 
                                ...
                                // This can be updated with an actual condition once the drivebase code is implemented.
