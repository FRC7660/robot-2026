...
class FuelPalantirLogicTest {
  ...
  private static Vision.CameraSnapshot snapshotWithTarget(
    ...
    return snapshotWithTarget(Cameras.BACK_LEFT, fiducialId, yawDeg, area, timestampSec);
  }
  ...
  private static Map<Cameras, Vision.CameraSnapshot> baseCameraData() {
    ...
    map.put(Cameras.BACK_LEFT, emptySnapshot(Cameras.BACK_LEFT));
    map.put(Cameras.FRONT_LEFT, emptySnapshot(Cameras.FRONT_LEFT));
    return map;
  }
  ...
  void fuelPalantir_locksCameraAndCommandsMotionWhenFuelSeen() {
    ...
    data.put(Cameras.FRONT_LEFT, snapshotWithTarget(Cameras.FRONT_LEFT, -1, 10.0, 1.0, 1.0));
    ...
    assertEquals(Cameras.FRONT_LEFT, step.nextState().lockedCamera().get());
  ...
  void fuelPalantir_stopsForwardWhenAreaAboveThreshold() {
    Map<Cameras, Vision.CameraSnapshot> data = baseCameraData();
    data.put(Cameras.BACK_LEFT, snapshotWithTarget(-1, 1.0, 5.0, 1.0));

    FuelPalantir.FuelPalantirStep step =
        FuelPalantir.fuelPalantir(
            data,
            new FuelPalantir.FuelPalantirState(Optional.of(Cameras.BACK_LEFT)),
            FuelPalantir.FuelPalantirMode.AUTONOMOUS,
            2.0);
  ...
  void chooseLockCamera_releasesLockWhenLockedCameraLosesTarget() {
    Map<Cameras, Vision.CameraSnapshot> data = baseCameraData();
    // BACK_LEFT is locked but has no targets; FRONT_LEFT has a fuel target
    data.put(Cameras.FRONT_LEFT, snapshotWithTarget(Cameras.FRONT_LEFT, -1, 5.0, 1.0, 1.0));

    Optional<Cameras> result = FuelPalantir.chooseLockCamera(data, Optional.of(Cameras.BACK_LEFT));

    assertTrue(result.isPresent());
    assertEquals(Cameras.FRONT_LEFT, result.get());
  }

  @Test
  void chooseLockCamera_keepsLockWhenLockedCameraStillSeesFuel() {
    Map<Cameras, Vision.CameraSnapshot> data = baseCameraData();
    data.put(Cameras.FRONT_LEFT, snapshotWithTarget(Cameras.FRONT_LEFT, -1, 3.0, 2.0, 1.0));
    data.put(Cameras.FRONT_LEFT, snapshotWithTarget(Cameras.FRONT_LEFT, -1, 1.0, 3.0, 1.0));

    Optional<Cameras> result = FuelPalantir.chooseLockCamera(data, Optional.of(Cameras.FRONT_LEFT));
  ...
  void fuelPalantir_keepsLockDuringBriefDetectionDropout() {
    ...
    assertEquals(Optional.of(Cameras.BACK_LEFT), step1.nextState().lockedCamera());
    assertEquals(Optional.of(Cameras.BACK_LEFT), step2.nextState().lockedCamera());
  }
  ...
  void fuelPalantir_slewLimitsForwardCommand() {
    Map<Cameras, Vision.CameraSnapshot> data = baseCameraData();
    data.put(Cameras.FRONT_LEFT, snapshotWithTarget(Cameras.FRONT_LEFT, -1, 0.0, 0.0, 1.0));
  ...
  void fuelPalantir_ballSeenNearCenter_stopsRotationSearch() {
    Map<Cameras, Vision.CameraSnapshot> data = baseCameraData();
    data.put(Cameras.FRONT_LEFT, snapshotWithTarget(Cameras.FRONT_LEFT, -1, 1.0, 1.0, 1.0));
  ...
  void fuelPalantir_ballSeenOffCenter_usesTrackingNotSearchRotation() {
    Map<Cameras, Vision.CameraSnapshot> data = baseCameraData();
    data.put(Cameras.FRONT_LEFT, snapshotWithTarget(Cameras.FRONT_LEFT, -1, 25.0, 1.0, 1.0));
  ...
  void fuelPalantir_afterSeeingBall_doesNotReturnToSearchSpinWhenLost() {
    ...
    seenData.put(Cameras.FRONT_LEFT, snapshotWithTarget(Cameras.FRONT_LEFT, -1, 12.0, 1.0, 1.0));
  ...
  @Test
  void fuelPalantir_afterLockingFrontCamera_doesNotSwitchToBackCamera() {
    Map<Cameras, Vision.CameraSnapshot> frontOnly = baseCameraData();
    frontOnly.put(Cameras.FRONT_LEFT, snapshotWithTarget(Cameras.FRONT_LEFT, -1, 10.0, 1.0, 1.0));

    FuelPalantir.FuelPalantirStep frontStep =
        FuelPalantir.fuelPalantir(
            frontOnly,
            new FuelPalantir.FuelPalantirState(Optional.empty(), 0, 0.0, 0.0, false),
            FuelPalantir.FuelPalantirMode.TELEOP,
            0.1);
    assertEquals(Optional.of(Cameras.FRONT_LEFT), frontStep.nextState().lockedCamera());

    Map<Cameras, Vision.CameraSnapshot> backOnly = baseCameraData();
    backOnly.put(Cameras.BACK_LEFT, snapshotWithTarget(Cameras.BACK_LEFT, -1, 5.0, 1.0, 1.0));

    FuelPalantir.FuelPalantirStep switchedStep =
        FuelPalantir.fuelPalantir(
            backOnly, frontStep.nextState(), FuelPalantir.FuelPalantirMode.TELEOP, 0.2);

    assertNotEquals(Optional.of(Cameras.BACK_LEFT), switchedStep.nextState().lockedCamera());
  }
}
